...
const Header = () => {
  ...
  return (
    <header className={`app-header ${scrolled ? 'scrolled' : ''}`}>
      <div className="logo">
        <a href="/index.html" className="logo-text" aria-label="Taj Design Consult - Homepage">Taj Design Consult</a>
      </div>
...
const CustomCursor = memo(() => {
    ...
    useEffect(() => {
        ...
        const hoverTargets = document.querySelectorAll(
            'a, button, [role="button"], input, select, textarea, label, .whatsapp-widget'
        );
...
const ApplicationForm = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        email: '',
        phone: '',
        position: careerOpenings[0].title,
        coverLetter: ''
    });
    ...
    const validate = (data: typeof formData, currentFileName: string) => {
        ...
        if (!data.coverLetter.trim() || data.coverLetter.length < 50) {
            errors.coverLetter = 'Cover Letter is required and must be at least 50 characters.';
        }
    ...
    useEffect(() => {
        if (Object.keys(touched).length > 0) {
            setFormErrors(validate(formData, fileName));
        }
    }, [formData, fileName, touched]);
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setTouched({
            name: true, email: true, phone: true, position: true, coverLetter: true, cv: true
        });
        ...
        if (Object.keys(currentErrors).length > 0) {
            ...
            if (firstErrorField) {
                 ...
                 if (firstErrorField === 'cv') {
                     elementToFocus = document.querySelector<HTMLLabelElement>('label[for="cv"]');
                 } else {
        ...
        console.log('CV File Name:', fileName);
                    ...
                    <label
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') {
                                e.preventDefault();
                                (document.getElementById('cv') as HTMLInputElement)?.click();
                            }
                        }}
                        className={`custom-file-upload ${touched.cv && formErrors.cv ? 'invalid' : ''}`}
                        aria-describedby={touched.cv && formErrors.cv ? 'cv-error' : undefined}>
